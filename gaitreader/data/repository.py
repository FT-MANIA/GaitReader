...
class RepositoryKinematicDataset(Dataset[KinematicSubjectSample]):
    ...
    def __init__(
        ...
        *,
        ...
        if method == "fixed_window":
            ...
            self.window_length = self.cycle_length
            self.num_cycles: int | None = (
                data.shape[-1] // self.cycle_length
            )
        else:
            segmenter = GaitParser.from_config(segmentation)
    ...
    def use_fixed_windows(self, window_length: int, word_length: int) -> None:
        """Replace words, preserving the already selected subjects and scaler."""
        if not 0 < window_length <= self.raw_data.shape[-1]:
            raise ValueError("fixed-window-length must be positive and no longer than the recording")
        reference = dict(self.segmentation_summary)
        self.segmentation_method = "fixed_window"
        self.window_length = window_length
        self.cycle_length = word_length
        self.num_cycles = self.raw_data.shape[-1] // window_length
        self._adaptive_cycles = None
        self.segmentation_summary = {
            "method": "fixed_window", "source_subjects": reference["source_subjects"],
            "retained_subjects": len(self), "excluded_subjects": reference["excluded_subjects"],
            "excluded_subject_details": self.excluded_subjects,
            "window_length_frames": window_length, "stride_frames": window_length,
            "target_length": word_length, "windows_per_side": self.num_cycles,
            "discarded_tail_frames": self.raw_data.shape[-1] % window_length,
            "standardization": self.standardizer.summary(),
            "cohort_selection": "same_adaptive_reference_as_control",
    ...
    def _side_cycles(self, side: torch.Tensor) -> torch.Tensor:
        ...
        usable = self.num_cycles * self.window_length
        cycles = side[:, :usable].reshape(6, self.num_cycles, self.window_length)
        cycles = cycles.permute(1, 0, 2).contiguous()
        if self.window_length != self.cycle_length:
            cycles = torch.nn.functional.interpolate(
                cycles, size=self.cycle_length, mode="linear", align_corners=True)
    ...
    def __getitem__(self, index: int) -> KinematicSubjectSample:
        ...
        if self._adaptive_cycles is None:
            ...
            boundaries = [[i * self.window_length, (i + 1) * self.window_length]
                          for i in range(self.num_cycles)]
            side_metadata = {
                "cycle_count": self.num_cycles, "boundaries": boundaries,
                "period_samples": self.window_length,
                "quality_scores": [1.0] * self.num_cycles,
                "quality_source": "constant_for_fixed_windows",
                "rejected_intervals": 0, "failure_reason": None,
            }
            segmentation_metadata = {
                "method": "fixed_window",
                "cycle_length": self.cycle_length,
                "window_length_frames": self.window_length,
                "left": dict(side_metadata), "right": dict(side_metadata),
            }
        else:
...
def build_repository_datasets(
    ...
    *,
    ...
    Subject quality control uses the same fixed bilateral minimum cycle count
    for every partition; it does not fit population-amplitude limits.
    """
    required = {
        "ssl_data",
        "ssl_label",
    ...
    if use_adaptive_quality_control:
        ...
        ssl_quality_control = AdaptiveKinematicQualityControl.from_config(quality_values)
        ...
        downstream_quality_control = ssl_quality_control
    else:
