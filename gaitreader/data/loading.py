...
def extract_subject_arrays(
    ...
    for person_id, subject_rows in frame.groupby("person_id", sort=False):
        sides = subject_rows["leg"].astype(str).str.strip().str.lower()
        if sides.duplicated().any():
            raise ValueError(f"Repeated side rows for {person_id}; use distinct paired-record IDs and a subject-group mapping.")
        side_rows = {
...
def _dataset_paths(config: dict[str, Any]) -> dict[str, Path]:
    """Resolve the three explicit CSV paths."""
