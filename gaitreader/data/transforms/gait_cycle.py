...
class GaitParser:
    ...
    The extension minimum shortly after each main swing-flexion peak is used as
    a reproducible kinematic cycle anchor.  These are kinematic proxy events
    rather than force-plate heel-strike annotations.
    """
    ...
    def __init__(
        ...
        *,
        ...
        peak_distance_fraction: float = 0.75,
        boundary_post_peak_min_fraction: float = 0.05,
        boundary_post_peak_max_fraction: float = 0.40,
        ...
        if not (
            0 <= boundary_post_peak_min_fraction
            < boundary_post_peak_max_fraction
            < 1
        ):
            raise ValueError(
                "post-peak boundary fractions must satisfy 0 <= min < max < 1"
            )
        ...
        self.boundary_post_peak_min_fraction = float(
            boundary_post_peak_min_fraction
        )
        self.boundary_post_peak_max_fraction = float(
            boundary_post_peak_max_fraction
        )
    ...
    def _flexion_boundaries(
        ...
        """Locate terminal-extension minima after main swing-flexion peaks."""
        dynamic_range = float(np.ptp(signal))
        prominence = max(
            np.finfo(float).eps,
            dynamic_range * self.peak_prominence_fraction,
            ...
            leading_start = max(
                ...
                int(
                    round(
                        first_peak
                        - period
                        * (1.0 - self.boundary_post_peak_min_fraction)
                    )
                ),
            )
            leading_end = max(
                ...
                int(
                    round(
                        first_peak
                        - period
                        * (1.0 - self.boundary_post_peak_max_fraction)
                    )
                ),
            )
            if leading_end - leading_start >= 2:
                leading = signal[leading_start : leading_end + 1]
                boundaries.append(leading_start + int(np.argmin(leading)))

            for peak in swing_peaks:
                search_start = min(
                    signal.size - 1,
                    int(
                        round(
                            peak
                            + period
                            * self.boundary_post_peak_min_fraction
                        )
                    ),
                )
                search_end = min(
                    ...
                    int(
                        round(
                            peak
                            + period
                            * self.boundary_post_peak_max_fraction
                        )
                    ),
                )
                if search_end - search_start >= 2:
                    post_peak = signal[search_start : search_end + 1]
                    boundaries.append(
                        search_start + int(np.argmin(post_peak))
                    )
